...
namespace DrakSolz.Tiles {
    public class LotteryMachine : ModItem {
        ...
        public override void SetStaticDefaults () {
            Tooltip.SetDefault ("Spend 2 gold for a chance to get lucky!");
        }
    ...
    public class LotteryMachineTile : ModTile {
        ...
        public override bool NewRightClick (int i, int j) {
            ...
            foreach (Item g in player.inventory) {
                if (g.type == ItemID.GoldCoin && g.stack >= 2) {
                    int c = (Main.rand.Next (500));
                    if (c >= 499) {  
                        ...
                        Item.NewItem (player.position, player.width, player.height, ItemID.SilverCoin, Main.rand.Next (99) + 1);
                        Main.PlaySound (SoundID.CoinPickup, player.position);
                        return true;
                    } else if (c >= 230) {  
                        Item.NewItem (player.position, player.width, player.height, ItemID.CopperCoin, Main.rand.Next (99) + 1);
                        Main.PlaySound (SoundID.CoinPickup, player.position);
                        return true;
                    } else {
