...
namespace DrakSolz.Projectiles {
    public class ChannelTProj : ModProjectile {
        ...
        public override void AI() {
            ...
            //projOwner.AddBuff(ModContent.BuffType<Buffs.ChannelBuff>(), 40);
            float distance = 325f;
                ...
                if (Main.player[k].active && Main.player[k].team == projOwner.team) {
                    if (projOwner.WithinRange(Main.player[k].Center, distance)) {
                        if (projOwner.setBonus.Contains("Channeler's Perfect Dance"))
                            Main.player[k].AddBuff(ModContent.BuffType<Buffs.PerfectChannelBuff>(), 40);
                        else Main.player[k].AddBuff(ModContent.BuffType<Buffs.ChannelBuff>(), 40);
                    }
                }
            }
