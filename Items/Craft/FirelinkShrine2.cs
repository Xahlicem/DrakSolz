...
namespace XahlicemMod.Items.Craft {
    public class FirelinkShrine2 : ModTile {
        ...
        public override void NearbyEffects(int i, int j, bool closer) {
            ...
            if (closer) {
                player.AddBuff(BuffID.Campfire, 320);
            }
        }
		...
		public override void ModifyWorldGenTasks(List<GenPass> tasks, ref float totalWeight)
		{
			...
			if (ResetIndex != -1)
			{
				tasks.Insert(ResetIndex + 1, new PassLegacy("Kindling the Flame", delegate (GenerationProgress progress)
				{
					...
					if (WorldGen.statueList.Any(point => point.X == mod.TileType<FirelinkShrine2>()))
					{
						return;
					}
					...
					for (int i = 0; i < WorldGen.statueList.Length; i++)
					{
						...
						WorldGen.statueList[i] = new Point16(mod.TileType<FirelinkShrine2>(), 0);
