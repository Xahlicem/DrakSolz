        ...
        public class SoulShatterGlobalNPC : GlobalNPC {
            public override void NPCLoot(NPC npc) {
                if (Main.rand.Next(15) == 0) {
                    if (npc.type == mod.NPCType<NPCs.Enemy.Endgame.Dungeon.Shinkage>() ) {
                        Item.NewItem((int) npc.position.X, (int) npc.position.Y, npc.width, npc.height, mod.ItemType<Items.Melee.ShinkagesKatana>(), 1);
                    }
                }
            }
        }
