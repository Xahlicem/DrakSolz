...
namespace DrakSolz.NPCs.Enemy.VoidPillar.Tiles {
	public class VoidMonolithTile : ModTile {
		...
		public override void KillMultiTile(int i, int j, int frameX, int frameY) {
			Item.NewItem(i * 16, j * 16, 32, 48, ModContent.ItemType<VoidItems.VoidMonolith>());
		}

		public override void NearbyEffects(int i, int j, bool closer) {
			if (Main.tile[i, j].frameY >= 56) {
				DrakSolzPlayer modPlayer = Main.LocalPlayer.GetModPlayer<DrakSolzPlayer>();
		...
		public override bool PreDraw(int i, int j, SpriteBatch spriteBatch) {
			...
			Main.spriteBatch.Draw(mod.GetTexture("NPCs/Enemy/VoidPillar/Tiles/VoidMonolithTile_Glowmask"), new Vector2(i * 16 - (int) Main.screenPosition.X, j * 16 - (int) Main.screenPosition.Y) + zero, new Rectangle(tile.frameX, tile.frameY + animate, 16, height), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
			return false;
		}
		...
		public override void MouseOver(int i, int j) {
			...
			player.showItemIcon2 = ModContent.ItemType<VoidItems.VoidMonolith>();
		}
