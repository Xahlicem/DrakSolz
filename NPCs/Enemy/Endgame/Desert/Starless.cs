...
namespace DrakSolz.NPCs.Enemy.Endgame.Desert {
    public class Starless : ModNPC {
        public override void SetStaticDefaults() {
            DisplayName.SetDefault("Starless");
        ...
        public override void AI() {
            ...
            if (AI_Timer == 20) {
                ...
                int pro = Projectile.NewProjectile(npc.Center.X + (40 * npc.direction), npc.Center.Y - 25, speed.X * 1.4f, speed.Y * 1.4f, ProjectileID.SandnadoHostileMark, npc.damage, 3f);
                Main.projectile[pro].friendly = false;
                Main.projectile[pro].hostile = true;
                Main.projectile[pro].tileCollide = false;

            }
            if (AI_Timer == 40 || AI_Timer == 60 || AI_Timer == 80 || AI_Timer == 100 ) {
                npc.TargetClosest(true);
                Vector2 speed = Main.player[npc.target].Center - npc.Center;
                DrakSolz.AdjustMagnitude(ref speed, 7.5f);
                int pro = Projectile.NewProjectile(npc.Center.X + (40 * npc.direction), npc.Center.Y - 25, speed.X * 1.4f, speed.Y * 1.4f, ProjectileID.AmberBolt, npc.damage, 3f);
                Main.projectile[pro].friendly = false;
                Main.projectile[pro].hostile = true;
                Main.projectile[pro].tileCollide = false;
